...
THIS_REPO='secure-wazuh'
SIBLINGS=('windows-wsus' 'pdq-deploy-inventory')
...
for s in "${SIBLINGS[@]}"; do
  ...
  # stronger than the sibling repositories', which reach implicitDeny for want of an Allow.
  assert "write ${s} tfstate"          explicitDeny s3:PutObject "arn:aws:s3:::${ACCOUNT}-terraform/${OWNER}/${s}/aws.tfstate" \
...
assert "dedicated tenancy"             implicitDeny ec2:RunInstances "${INST}" "ec2:InstanceType=string=m6i.xlarge" \
