...
readonly THIS_REPO='secure-wazuh'
...
readonly -a FOREIGN_IDENTITY_PATTERNS=(
    ...
    '1316209092'              # windows-wsus repository id
    '1316209027'              # pdq-deploy-inventory repository id
)

# DELIBERATELY ABSENT above: 'wazuh', 'secure-wazuh', and this repository's own id. They are this
# repository's IDENTITY, not foreign literals. A verbatim copy of a sibling's gate lists them and
# then rejects this repo's own artifact prefixes (functions/wazuh/*, applications/wazuh-agent/*),
# its role names, and its own trust documents. 'aws-marketplace' is likewise absent: this repo's
# RHEL-family AMI resolves to that owner alias, so it is a required value here, not a finding.
